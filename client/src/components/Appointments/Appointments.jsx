...
const Appointments = () => {
  ...
  const getUpcomingAppointments = async () => {
    try {
      const response = await axios.get(
        `http://localhost:1128/api/appointment/appointment/upcoming/${lawyer.id}`
      );
  ...
  const deleteAppointment = async (id) => {
    ...
    try {
      const x = await axios.delete(
        `http://localhost:1128/api/appointment/deleteAppointment/${id}`
      );
      setRefrech(!refrech);
      console.log("deleted");
    } catch (error) {
  ...
  const acceptAppointment = async (appointmentId, lawyerId, userId) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:1128/api/appointment/updateAppointment/${appointmentId}`,
        { accepted: "accepted" }
      );
      ...
      const checkUser = await axios.get(
        `http://localhost:1128/api/user_lawyer/getUserLawyerId/${userId}/${lawyerId}`
      );
  ...
  const getAcceptedAppointments = async () => {
    try {
      const response = await axios.get(
        `http://localhost:1128/api/appointment/appointment/lawyer/${lawyer.id}`
      );
  ...
  const getUserById = async (userId) => {
    try {
      const response = await axios.get(
        `http://localhost:1128/api/user/getUserId/${userId}`
      );
                  ...
                  <div className="cardAndBorder">
                    {upcomingAppointments.map((appointment) => (
                      <div
                        ...
                        style={{ borderRadius: "3%", justifyContent: "center" }}
                      >
                        <div className="appointment1-text">
                          ...
                          ></img>
                          <p className="appointment-username">
                            {appointment?.user.fullName}
                          </p>
                          <p className="appointment-date">
                            Date: {appointment.date.slice(0, 10)} Time:{" "}
