...
const fetchFolder = async()=> {
  try {
    const response = await axios.get("http://localhost:1128/api/folder/getAll/folder")
...
const fetchFile = async()=> {
  try {
    const response = await axios.get("http://localhost:1128/api/file/getAll/files")
    console.log(response.data);
...
useEffect(()=>{
  ...
  const addFolder = async ()=>{
    try {
       const response = await axios.post("http://localhost:1128/api/folder/add" , {
        name:newName,
       })
       setRefrech(!refrech)
    } catch (error) {
  ...
  <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/8/87/PDF_file_icon.svg/1667px-PDF_file_icon.svg.png" alt="W3Schools" width="104" height="142" />
            ...
            <div className="block-folder">
              <div style={{display:'flex', justifyContent:'space-between', padding:'1rem' , overflowY:'scrol'}}>
                <p style={{fontSize:'20px'}}> Folder Storage </p>
                 <button style={{border:'none', width:'40px', borderRadius:'50%', fontSize:'30px', backgroundColor:'goldenrod'}} onClick={()=>{addFolder()}}> + </button>
              </div>
              <div style={{display:'flex', flexDirection:'column', gap:'1rem', paddingLeft:'2rem' }}>
                {folders.map((folder,i)=>{
                  return (
                    <>
                    <div style={{display:'flex', flexDirection:'row' , alignItems:'center', alignText:'center', height:'3rem' }} key={i}> 
                    <FolderIcon style={{color:"goldenrod", height:"100%", width:'20%'}}/>
                     <p style={{height:'50%' , margin:0}}>{folder.name} </p>
                    </div>
            ...
            <div className="block-note">
              <div style={{display:'flex', justifyContent:'space-between', padding:'1rem'}}>
                <p style={{fontSize:'20px', fontWeight:'bold'}}>Add Notes</p>
                <button style={{border:'none', borderRadius:'50%', backgroundColor:'goldenrod', width:'40px', fontSize:'30px'}}> + </button>


              </div>
              <div style={{display:'flex', justifyContent:'center'}}>
              <div style={{width:'47vh', height:'15vh', display:'flex', }}>
              <div class="card">
  <div class="comments">
    ...
    <div class="box-container" >
      ...
      <div>
        <div class="formatting" style={{display:'flex', justifyContent:'flex-end'}} >
            ...
            <div style={{display:'flex', justifyContent:'flex-end'}}>
              ...
              <div style={{display:'flex',  padding:'1rem' , flexDirection:"column",overflowY:'scrol'}}>
                ...
                <p style={{fontSize:'20px', margin:0}}> File Storage </p>
                 <button style={{border:'none', width:'40px', borderRadius:'50%', fontSize:'30px', backgroundColor:'goldenrod'}} onClick={(e)=>{addFile()}}> + </button>
                 <input type="file"  style={{position:"absolute",right:"17rem", width:"10%", opacity:0}} onChange={(e)=>{getFile(e)}}/>
                </div>
             ...
             <a src={files} style={{color:"goldenrod", height:"100%", width:'20%'}}/>
              <p style={{height:'50%' , margin:0}}>{files.name} </p>
             </div>
             <div style={{height:'.01rem', width:'80%', backgroundColor:'grey' }}></div>
