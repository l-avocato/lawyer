 ...
 const onConnect = useCallback(
  (params) => {
    setEdges((prevEdges) => {
      ...
      axios.post("http://localhost:1128/api/edge/add", {
        label: "",
  ...
  const fetchData =  async ()=>{
   ...
   if(result.data.length){  const newNodes= result.data.map((data) => {
        return ( 
  ...
  const fetchEdge =  async ()=>{
    try {
      const result = await axios.get("http://localhost:1128/api/edge/getAll/edge")
  ...
  useEffect(() => {
    fetchData()
    fetchEdge()
    console.log("this is the selected node ",selectedEdge?.id)
  ...
  const updateNodeName = async (id) => {
    try {
    await axios.put(
        `http://localhost:1128/api/phase/update/${id}`,
        { label: newLabel }
  ...
  const onNodeDragStop = (event, node) => {
    ...
    axios.put(`http://localhost:1128/api/phase/update/${id}`, {
              ...
              <div className="modal-dialog">
                <div className="modal-content">
                  ...
                  <div className="modal-body">
                    ...
                    <input name="text" class="input" placeholder="Update Edge Name" style={{width:'500px', borderRadius: '0.75rem', padding:'0.5rem', marginLeft:'4rem', border:'2px solid #f0f0f0', transition:' all 0.2s ease-in-out'}}
                     onChange={(e)=>{setNewLabel(e.target.value)}}
                    ></input>
                  </div>
                  <div className="modal-footer" style={{display:'flex', justifyContent:"space-between" , marginLeft : '1rem'}}>
                  
                   
                    
                    <DeleteForeverIcon style={{color:'gold', height:'30px', width:'30px', borderRadius:'50%', cursor:'pointer'}} onClick={()=>{deleteNode(selectedEdge.id)
                    console.log(selectedEdge);
                    setShowEdgeModal(false)
                    }} />
                    <EditNoteIcon style={{color:'gold', height:'30px', width:'30px', cursor:'pointer'}} onClick={()=>{updateNodeName(selectedEdge.id)
                    setShowEdgeModal(false)
                    }}/>
                    

                    <button type="button" style={{height: "50px", width:'150px', transition:'0.2s'}}>
                      See All details
                    </button>
                  </div>
                </div>
