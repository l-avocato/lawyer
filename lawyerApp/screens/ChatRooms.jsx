...
const ChatRooms = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      getConversations(); // Update conversations when a new message is received
    });
...
<ScrollView>
  {conversations.map((conversation) => {
    const lastMessage = messages[conversation.id]?.[0]?.text;
    return (
      <TouchableOpacity
        key={conversation.id}
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.username}>{conversation.lawyerName}</Text>
